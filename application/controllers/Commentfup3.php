...
class Commentfup3 extends CI_Controller
{
    function __construct()
    {
        parent::__construct();
        $this->load->model("Commentfup3_model", "commentfup3");
    }
    ...
    public function index()
    {
        ...
        if ($_POST) {
            $this->commentfup3->add_comment($_POST);
            redirect(site_url() . "fup3/read/" . $this->input->post('id_fup'));
        } else {
    ...
    {
        if (!empty($id)) {
            $this->commentfup3->delete_comment($id);
            redirect(site_url() . "fup3/read/" . $id_fup);
        }
        echo "Ada yang salah..!!";
    }
}

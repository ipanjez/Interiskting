...
class Commentfup_model extends CI_Model
{
    ...
    {
        $this->db->insert('commentfup', $data);
    }
    ...
    //public function hae_kommentit($id)
    {
        ...
        //poistoa varten
        //untuk dihapus
        $this->db->select('*, commentfup.id as id');
        $this->db->from('commentfup');
        $this->db->join('users', 'users.id = commentfup.id_users');

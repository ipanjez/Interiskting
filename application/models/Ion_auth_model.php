...
class Ion_auth_model extends CI_Model
{
	...
	public function login($identity, $password, $remember = FALSE)
	{
		...
		$this->trigger_events('extra_where');
		//modif ke usersession company dan phone untuk unit kerja dan npk, company dan phone
		$query = $this->db->select($this->identity_column . ', email, id, password, active, last_login, unit_kerja, npk')
			->where($this->identity_column, $identity)
			//ermail dan npk -> pake phone
			->or_where('npk', $identity)
			->limit(1)
			->order_by('id', 'desc')
			->get($this->tables['users']);
	...
	public function set_session($user)
	{
		...
		$session_data = array(
			...
			//modif ke usersession company dan phone untuk unit kerja dan npk
